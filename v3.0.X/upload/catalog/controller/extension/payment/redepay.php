...
class ControllerExtensionPaymentRedePay extends Controller {
	...
	private function getDocument($order_info, $config) {
		...
		if(is_numeric($config->config_document)) {
			...
			if(isset($order_info["custom_field"][$config->config_document])){
				$document = $order_info["custom_field"][$config->config_document];
			}else{
				echo "Campo documento invalido";
			}
		}
		return $document;
	}
	...
	private function getConfig() {
		...
		$config->status = $this->config->get("payment_redepay_status");
		$config->max_installments = $this->config->get("payment_redepay_max_installments");
		$config->api_key = $this->config->get("payment_redepay_api_key");
		$config->token_nip = $this->config->get("payment_redepay_token_nip");
		$config->public_key = $this->config->get("payment_redepay_public_token");
		$config->config_document = $this->config->get("payment_redepay_document");
		$config->config_address = $this->config->get("payment_redepay_address");
		$config->config_number = $this->config->get("payment_redepay_number");
		$config->config_complement = $this->config->get("payment_redepay_complement");
		$config->config_neighborhood = $this->config->get("payment_redepay_neighborhood");
		$config->config_phone = $this->config->get("payment_redepay_phone");
		$config->config_cellphone = $this->config->get("payment_redepay_cellphone");
		$config->notification_url = $this->config->get("payment_redepay_notification_url");
		$config->redirect_url = $this->config->get("payment_redepay_redirect_url");
		$config->cancel_url = $this->config->get("payment_redepay_cancel_url");
		$config->min_value_installment = $this->config->get("payment_redepay_min_value_installment");
		$config->min_installment_value = $this->config->get("payment_redepay_min_installment_value");
		return $config;
	}
}
