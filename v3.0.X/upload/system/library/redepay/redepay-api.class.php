...
class RedePayAPI {
		...
		$parameters = array(
			...
			"customer" => array(
				...
				"documents" => array(
					0 => array(
						"kind" => "cpf",
						"number" => $this->getDocument($properties, $config)
					)
				),
	...
	private function getDocument($properties, $config) {
		return strval(preg_replace('/[^0-9]/', '', $properties['custom_field'][2]));
	}
	...
	private function getShippingInformation($properties){
		...
		$shipping['address'] = array(
			...
			"postalCode" => preg_replace('/[^0-9]/', '', $properties['shipping_postcode']),
